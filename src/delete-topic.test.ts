...
describe("DELETE /api/topics/:id", () => {
  ...
  test("deletes topic and cascades to ideas", async () => {
    const topicId = db.query("SELECT id FROM topics WHERE name = 'Test Topic'").get() as { id: string } | undefined;
    expect(topicId).toBeDefined();

    const topicBeforeDelete = db.query("SELECT isArchived FROM topics WHERE id = ?").get(topicId!.id);
    expect((topicBeforeDelete as { isArchived: number }).isArchived).toBe(0);

    const ideasBeforeDelete = db.query("SELECT id, isArchived FROM ideas WHERE topicId = ?").all(topicId!.id) as Array<{ id: string; isArchived: number }>;
    ...
    const ideasAfterDelete = db.query("SELECT id, isArchived FROM ideas WHERE topicId = ?").all(topicId!.id) as Array<{ id: string; isArchived: number }>;
