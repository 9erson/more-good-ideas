...
describe("database schema", () => {
  ...
  test("topics table exists and has correct schema", () => {
    const result = db
      .query("SELECT sql FROM sqlite_master WHERE type='table' AND name='topics'")
      .get() as { sql: string }
  ...
  test("ideas table exists and has correct schema", () => {
    const result = db
      .query("SELECT sql FROM sqlite_master WHERE type='table' AND name='ideas'")
      .get() as { sql: string }
    expect(result).toBeDefined()
    expect(result.sql).toContain("id TEXT PRIMARY KEY")
  ...
  test("tags table exists and has correct schema", () => {
    const result = db
      .query("SELECT sql FROM sqlite_master WHERE type='table' AND name='tags'")
      .get() as { sql: string }
    expect(result).toBeDefined()
    expect(result.sql).toContain("id TEXT PRIMARY KEY")
  ...
  test("topic_tags table exists and has correct schema", () => {
    const result = db
      .query("SELECT sql FROM sqlite_master WHERE type='table' AND name='topic_tags'")
      .get() as { sql: string }
    expect(result).toBeDefined()
  ...
  test("idea_tags table exists and has correct schema", () => {
    const result = db
      .query("SELECT sql FROM sqlite_master WHERE type='table' AND name='idea_tags'")
      .get() as { sql: string }
    expect(result).toBeDefined()
  ...
  test("feedback table exists and has correct schema", () => {
    const result = db
      .query("SELECT sql FROM sqlite_master WHERE type='table' AND name='feedback'")
      .get() as { sql: string }
    expect(result).toBeDefined()
    expect(result.sql).toContain("id TEXT PRIMARY KEY")
  ...
  test("foreign key constraint enforced: deleting topic with ideas cascades", () => {
    ...
    const topicTagBeforeDelete = db
      .query("SELECT * FROM topic_tags WHERE topicId = ? AND tagId = ?")
      .get(topicId, tagId)
    expect(topicTagBeforeDelete).toBeDefined()

    const ideaTagBeforeDelete = db
      .query("SELECT * FROM idea_tags WHERE ideaId = ? AND tagId = ?")
      .get(ideaId, tagId)
    ...
    const topicTagAfterDelete = db
      .query("SELECT * FROM topic_tags WHERE topicId = ? AND tagId = ?")
      .get(topicId, tagId)
    expect(topicTagAfterDelete).toBeNull()

    const ideaTagAfterDelete = db
      .query("SELECT * FROM idea_tags WHERE ideaId = ? AND tagId = ?")
      .get(ideaId, tagId)
  ...
  test("full workflow: topic with idea, tags, and feedback", () => {
    ...
    const topicTags = db
      .query("SELECT COUNT(*) as count FROM topic_tags WHERE topicId = ?")
      .get(topicId)
    expect((topicTags as { count: number }).count).toBe(2)

    const ideaTags = db
      .query("SELECT COUNT(*) as count FROM idea_tags WHERE ideaId = ?")
      .get(ideaId)
